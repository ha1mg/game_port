...
770 IF E<1 OR E>10 OR F<1 OR F>10 THEN 760
780 Z=1
790 I(E,F)=1
800 H(A,B)=1
810 PRINT
...
840 IF M<1 OR M>10 OR N<1 OR N>10 THEN 830
850 INPUT "WHERE TO MOVE TO";S,T
860 IF S<1 OR S>10 OR T<1 OR T>10 THEN 850
870 IF I(S,T) = 1 THEN 850
880 FOR X=1 TO 8
890     IF P(X,1)+E=S AND P(X,2)+F=T THEN 920
900 NEXT X
910 GOTO 850
920 PRINT
930 L=1
940 FOR X=1 TO 8
950     IF P(X,1)+E>10 OR P(X,1)+E<1 OR P(X,2)+F>10 OR P(X,2)+F<1 THEN 1000
960     IF  I(P(X,1)+E,P(X,2)+F)=1 THEN 1000
970     T(L,1)=P(X,1)+E
980     T(L,2)=P(X,2)+F
990     L=L+1
1000 NEXT X
...
1090 L=1
1100 FOR X=1 TO 8
1110    IF P(X,1)+A>10 OR P(X,1)+A<1 OR P(X,2)+B>10 OR P(X,2)+B<1 THEN 1160
1120    IF H(P(X,1)+A,P(X,2)+B)=1 THEN 1160
1130    T(L,1)=P(X,1)+A
1140    T(L,2)=P(X,2)+B
1150    L=L+1
1160 NEXT X
...
1330 IF M<>J OR D<>T THEN 1390
1340 PRINT C$"HEY! YOU GOT HIM!!"
1350 Z=0
1360 IF C<>S OR D<>T THEN 1390
...
1420 B=K
1430 FOR X=1 TO 8
1440    IF P(X,1)+E>10 OR P(X,1)+E<1 OR P(X,2)+F>10 OR P(X,2)+F<1 THEN 1460
1450    IF I(P(X,1)+E,P(X,2)+F)=0 THEN 1490
1460 NEXT X
...
1510 PRINT G$;S$;G$
1520 FOR X=1 TO 10
1530    IF X=10 THEN 1560
1540    PRINT X;
1550    GOTO 1570
1560    PRINT "10 ";
1570    FOR Y=1 TO 10
1580        IF I(X,Y)=1 THEN 1610
1590        PRINT ":";
1600        GOTO 1620
1610        PRINT "*";
1620    NEXT Y
1630    PRINT " ";S$;
1640    IF X=10 THEN 1670
1650    PRINT X;
1660    GOTO 1680
1670    PRINT "10 ";
1680    FOR Y=1 TO 10
1690        IF H(X,Y)=1 THEN 1720
1700        PRINT ":";
1710        GOTO 1730
1720        PRINT "*";
1730    NEXT Y
